    ...
    this.el.querySelectorAll('img').forEach(img => {
      img.classList.forEach(cls => {
        if (cls.startsWith("wp-image-")) {
          this.elements.set(img.closest("figure"), Number.parseInt(cls.substr(9),10))
        }
      })
    })
    ...
    if (this.elements.size > 0 && !this.medias) {
      ...
      try {
        this.elements.forEach(async (id, key) => {
          ...
          let mediaInst = await (new Query(this.connection, new LinkedQueryArgs(Media, new Route("media"), id))).result
